...
def giniGain(S,attribute):
    ...
    sum = 0
    for i in range(len(counts)):
        sum += (counts[counts.index[i]]/total)*giniIndex(S.loc[S[attribute] == counts.index[i]])
...
def buildTree(examples,attributes,depth,depthLimit):
    ...
    if not attributes or (depth >= depthLimit) or (len(examples) <= 51):
        ...
        label = examples['decision'].value_counts(ascending=True).index.tolist()[-1]
        return {'decision':label}
    else:
        attribute , newAttributes= bestAttribute(examples,attributes) #attribute A has 2 possible values
        #Create an internal node with 2 children
        node = {attribute: {0: buildTree(examples.loc[examples[attribute] == 0],newAttributes,depth+1,depthLimit), 1: buildTree(examples.loc[examples[attribute] == 1], newAttributes, depth+1,depthLimit)}}
        return node
############# Bagging #####################
...
if __name__ == '__main__':
    if len(sys.argv) > 4:
        print("Usage: python trees.py [trainingDataFilename] [testDataFilename] [modelIDX]")
