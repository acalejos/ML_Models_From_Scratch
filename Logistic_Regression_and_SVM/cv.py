        ...
        for t in t_frac:
            for idx in range(1,10):
                ...
                if s == 0:
                    ...
                    testResult = round(lr_svm.test(test_set,model,1,.5,"LR Testing"),2)
                    accuracies[0][t].append(testResult)
                if s == 1:
                    model = lr_svm.train_svm(train_set)
                    testResult = round(lr_svm.test(test_set,model,2,.5,"SVM Testing"),2)
                    accuracies[1][t].append(testResult)
                if s == 2:
                    (probs,headers) = nb.trainModel(1.0,train_set)
                    testResult = round(nb.test(test_set,probs,headers,"NBC Testing"),2)
...
def plot_data(accuracies, sizes):
    ...
    plt.plot(x,Y[0],c='r',marker="o",fillstyle = 'none',label='Logistic Regression Model Accuracy')
    plt.plot(x,Y[1],c='b',marker="o",fillstyle = 'none',label='SVM Model Accuracy')
    plt.plot(x,Y[2],c='m',marker="o",fillstyle = 'none',label='NBC Model Accuracy')
...
if __name__ == '__main__':
    if len(sys.argv) > 4:
        print("Usage: python cv.py [lr_training_ data] [svm_training_data] [nbc_training_data]")
        #sys.exit(0)
    else:
        main(sys.argv[1:])
